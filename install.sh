...
level1(){
  prepare
  SELECTED=(1 2 3 17 18)
  info "LEVEL 1 :: BASIC SHIELD"
  install_indices
  post_install
}
level2(){
...
level3(){
  prepare
  SELECTED=()
  for i in {1..24}; do SELECTED+=("$i"); done
  info "LEVEL 3 :: FULL SHIELD"
  install_indices
  post_install
}
...
menu(){
  ...
  while true; do
    printf '\n'
    printf "${CYAN}[@]${NC} Protection Menu\n\n"
    printf "  ${GREEN}[1]${NC} LEVEL 1  :: BASIC SHIELD\n"
    printf "  ${YELLOW}[2]${NC} LEVEL 2  :: HARD SHIELD\n"
    printf "  ${MAGENTA}[3]${NC} LEVEL 3  :: FULL SHIELD\n"
    printf "  ${RED}[4]${NC} UNINSTALL PROTECT\n"
    ...
    case "$choice" in
      ...
      2) level2; break ;;
      3) level3; break ;;
      4) [[ -x "$SELF_UNINSTALL" ]] && "$SELF_UNINSTALL" || warn "Uninstaller belum tersedia."; break ;;
      0) exit 0 ;;
...
case "${1:-menu}" in
  1|protect1|level1) banner; auth; level1 ;;
  2|protect2|level2) banner; auth; level2 ;;
  3|protect3|level3) banner; auth; level3 ;;
  4|uninstall) [[ -x "$SELF_UNINSTALL" ]] && "$SELF_UNINSTALL" || die "Uninstaller belum tersedia." ;;
  menu) menu ;;
  *) echo "Usage: $0 [1|2|3|4|menu|uninstall]"; exit 1 ;;
esac
